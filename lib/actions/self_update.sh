#!/usr/bin/env bash
# actions/self_update.sh — tự cập nhật bộ lệnh iflmmo.
# Ưu tiên git pull (nếu /opt/wp-factory là git repo); fallback tải tarball.
...
IFLMMO_TARBALL_URL="${IFLMMO_TARBALL_URL:-https://cdn.lat.vn/wp-factory.tar.gz}"
...
_syntax_ok() {
  ...
  done < <(find "$root" \( -name '*.sh' -o -path '*/bin/iflmmo' \) -type f)
  return 0
}
...
act_self_update() {
  ...
  if [ -d "${WPF_ROOT}/.git" ] && need_cmd git; then
    ...
    if git -C "$WPF_ROOT" pull --ff-only >/dev/null 2>&1; then
      ...
      chmod +x "${WPF_ROOT}/bin/iflmmo" 2>/dev/null || true
      ln -sf "${WPF_ROOT}/bin/iflmmo" /usr/local/bin/iflmmo
      ui_msg "Đã cập nhật iflmmo qua git.\nPhiên bản: $(_current_version)\nChạy lại 'iflmmo' để dùng bản mới."
      return 0
    fi
  ...
  # Fallback tarball
  info "Tải bản mới từ ${IFLMMO_TARBALL_URL}..."
  local tmp; tmp="$(mktemp -d)"
  if ! curl -fsSL "$IFLMMO_TARBALL_URL" -o "${tmp}/wp-factory.tar.gz"; then
    warn "Tải tarball thất bại."; rm -rf "$tmp"; return 1
  fi
  tar -C "$tmp" -xzf "${tmp}/wp-factory.tar.gz" || { warn "Giải nén lỗi."; rm -rf "$tmp"; return 1; }
  # tarball giả định giải nén ra thư mục wp-factory/
  local newroot; newroot="$(find "$tmp" -maxdepth 2 -name VERSION -printf '%h\n' | head -1)"
  [ -n "$newroot" ] || newroot="${tmp}/wp-factory"
